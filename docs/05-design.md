...
| Rating starts from your rank | ~~`seedFromRank`, seeded from the rank the client reads (M1.3, M2.2)~~ — **no longer true at all** (M7.19, 2026-09-16): every stored rating starts at `provisionalSeed()`, `{ mu: 20, sigma: 12 }` — 1200 — for everybody, and `seedFromRank` has one caller left, the balancer's live guess for a face with no customs games, which is never persisted. The shipped line in `lib/shellCopy.ts` still says "starts from your rank" on every page of the site; **product owes the re-word**, along with `/p/[puuid]`'s `Seeded from Gold IV at 1200.`, and the two should be written together since they are the same claim in two places |
