...
- the window's **range and count** — `Sunday 6 Sep to Saturday 12 Sep · 14 rated games` on `/leaderboard`
  ...
  M5.15's seed line already ends `, 6 games since.` **`/stats` prints the whole form**, range and count, in the
...
reader sees above the sections is the seed line's, and it is still worded `games` — see the copy-table row, and
**M7.22**, which is where naming it is decided.
