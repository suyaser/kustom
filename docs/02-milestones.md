...
| M2 Companion v1: roster and results | in progress | M2.1 to M2.5, M2.7 to M2.10, M2.13 to M2.15, M2.18 to M2.20 done; M2.6 built as Kustom.exe 0.1.3 (apps/companion/dist, sha256 ffe6345e…), publish (v0.1.3, plus the kustom-releases README rename) and the Windows run pending on the user. **0.1.5 released 2026-09-15** (the user; `pnpm --filter companion release`, tag published on `kustom-releases`), the bump M4.1's gate flip owed, carrying `LOBBY_WRITE_VERIFICATION.verified: true`. **Lobby control itself has not been tested live yet** — the release exists, but nobody has run it against a real lobby and watched the writes actually go through; that is still owed before M4's flip counts as landed for the group. M2 ticks after Session 2 of docs/06-test-night.md. |
