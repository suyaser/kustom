...
- [x] **M5.34** (landed 2026-09-15: `/1v1` reads the same `gateRatedGame` universe `/stats` uses. Each lane lists the five longest same-role opposite-side series past three meetings; Lane bully is 70% over five; Dead heat is a score within one game over five. `?a=` and `?b=` pick any two people for enemies, allies, same-lane, KDA, form and a verdict. Default window `All time`. Rift only. **Does not change `/stats`, the awards, or the rating model.**) Lane 1v1 and head-to-head.
