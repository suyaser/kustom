...
- [x] **M7.18** (landed 2026-09-16, `web-engineer`: **no number, query, gate or loader changed** — words beside
  ...
  formatter itself stays — `boardSlotLine` is its caller). **Only acceptance 3's second clause landed**: the one
  sentence above the stats sections. The rated count a reader can actually see on the page is the **seed line's**
  `, 37 games since.` — `player.games`, worded as plain `games` directly above played-count sections — which is
  exactly the ambiguity the task was opened to fix and is still unlabelled. Naming it means editing an
  M7.19-pinned string or inverting M5.22, so it is product's word choice and the designer's placement: **open as
  M7.22**.)
  ...
  them. *(owner: `web-engineer`, with the designer for placement; after M7.1 ✓ and M7.11 ✓)*
    ...
    >    **(Landed half-met, 2026-09-16.** The second clause is on the page. The first is **not**: M5.22 leaves
    >    the header record with no count on any window, so there is nothing there to name, and the branch that
    >    pretended otherwise was unreachable code and has been removed. Carried to **M7.22**, which decides
    >    whether the seed line's own count gains the word or M5.22's rule is inverted.)**
...
- [ ] **M7.22** The one count on `/p/[puuid]` that still does not say what it counted. M7.18 named the board's
  ...
  *(owner: `product` (the word choice), then `designer` (placement, if option (c)), then `web-engineer`; after
  M7.18 ✓ and M7.19 ✓; **blocked on a product/design decision** — copy and placement only: no rating, no fold,
  no loader, no query, no rebuild, no migration)*
