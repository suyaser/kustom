...
| M2 Companion v1: roster and results | in progress | M2.1 to M2.5, M2.7 to M2.10, M2.13 to M2.15, M2.18 to M2.20 done; M2.6 built as Kustom.exe 0.1.3 (apps/companion/dist, sha256 ffe6345e…), publish (v0.1.3, plus the kustom-releases README rename) and the Windows run pending on the user. **0.1.5 released 2026-09-15** (the user; `pnpm --filter companion release`, tag published on `kustom-releases`), the bump M4.1's gate flip owed, carrying `LOBBY_WRITE_VERIFICATION.verified: true`. `apps/companion/package.json`'s own version number caught up to the published 0.1.5 on 2026-09-16 (it had stayed uncommitted locally since the release). The release itself hasn't been run against a real lobby yet — see M4's row for what that observation would and wouldn't confirm. M2 ticks after Session 2 of docs/06-test-night.md. **M2.11 (Windows verification pass) deferred to next release by the user, 2026-09-15** — needs a real Windows/League session that isn't happening for this release. |
...
- [x] **M4.11** (landed 2026-09-15, reviewed clean, closes M4.3 acceptance 7: the tonight snapshot now carries each member's `lobby_members.side` and each chosen seat's `liveSide`; `anySeatOnTheWrongSide` decides the line, absent only when all ten report a side and every one matches, present for a seat nobody has placed too — the same case `switchSide.ts` skips, which is why the page stays louder than the queue. No new subscription: the existing whole-page Realtime re-read already fires on a `lobby_members` change, and the "no re-render of the team cards" half of the brief turned out to be a property of the markup — `TeamCard` reads nothing derived from `liveSide`, proven with real `innerHTML` compared `toBe`-identical between a matched and mismatched render, not merely asserted. Cross-checked against the server's own `switchSideMoves` so the page and the command queue cannot drift on the null-side case. Still owed: a `05-design.md` correction — the line's copy section still says the vanish is unconfirmed and the deferral is live — and a decision row for the null-keeps-it-up rule, which the 2026-09-11 rows don't cover.) The side line goes when the sides are right (product, 2026-09-11; M4.3 acceptance 7 stays unmet until this lands). The tonight snapshot carries each chosen player's current `lobby_members.side`; the side line is present while any of the ten is mismatched and absent when all ten match, driven by the Realtime `lobby_members` events with no refetch and no re-render of the team cards. Owner: `web-engineer`, with the gate flip. **Acceptance:** a component test with ten matched seats renders no `.cn-side-line`; one mismatched seat renders it; the cards' markup is byte-identical between the two.
...
- [ ] **M5.6** (**deferred to next release, the user, 2026-09-15** — do not dispatch) Find out how far back match history goes. M0 only ever read the default window (`begIndex=0&endIndex=20`, which returned 21 games, inclusive) and never paged past it, so the reach of backfill is a guess. Walk `begIndex` back in pages of 20 on a real client until the client stops returning games or starts erroring, and write the answer into `03-lcu-reference.md`: how many games deep it goes, whether `gameCount` is the true total or just the window, and what an over-the-end request does (empty `games[]`, 400, or a repeat of the last page).
...
- [x] **M7.3** (landed 2026-09-15: `foldWeeklyRatings` in `apps/web/lib/board/weekly.ts` folds `this-week`/`last-week` at read time from the stored seed, through that week's rated games only — no table, no migration. The board sorts and prints the weekly `Rating` on those two windows, never Proven; `All time` and the month windows are byte-identical to before. Verified: a clean 2W-0L week outranks a 4W-4L week even though the shorter week's `ordinal` is lower — the exact fix the milestone exists for. The rail, the tonight page, and both Discord posts all read the same `track`-tagged rows. A week row's expand now shows the weekly deltas, not the stored ones, so the numbers add up. Known, scoped-out gap: `/p/[puuid]` still shows the all-time number on a week window — follow-up approved 2026-09-15, **closed by M7.16, 2026-09-16**.) The weekly board reads the weekly rating. `This week` and `Last week` are folded from scratch at read time, seeded from rank, through that week's rated games only. All-time, the month windows and team-forming are untouched. *(owner: `platform-engineer`; after M7.1 and M7.2)*
    ...
    >
    > - **Both strings below were re-worded by M7.21 on 2026-09-16** — `back at their rank` became `on the same
    >   rating`, because M7.19 took the League rank out of every seed. They are left here as written, because a
    >   landed brief is a record; **M7.21 carries the live text**. Everything else on this page still holds.
...
- [x] **M7.9** (landed 2026-09-15: `foldGame` in `apps/web/lib/ingest/fold.ts` now runs `rateGame` → the new exported `gameAward(players, winningSide)` → `applyMvpAceBonus`, one implementation called by both `rating.ts` (live) and `rebuild.ts` (backfill) off the identical ten-column select — `role, kills, deaths, assists, gold, damage_to_champs, cs, vision_score, damage_self_mitigated, damage_to_objectives`. No new column, no stored marker: `gameAward` is exported specifically so M7.10 can name the winner without folding a second copy of the formula into the web app. Reviewed clean after two rounds — the code needed no rework, only two doc gaps the reviewer caught: acceptance 6's premise was wrong (waived, see its own line above) and two decision rows were owed (`04-decisions.md`, 2026-09-15). `pnpm -r typecheck`, `pnpm -r test` (web 1472/1472, run twice) and `pnpm --filter web build` all pass on the merge onto main. Live from this merge for new games; stored history stays unadjusted until M7.11's one rebuild.) Apply the MVP / ACE bonus in the fold, once, for both callers. *(owner: `platform-engineer`; after M7.7 and M7.8, and land it in the same session as M7.7's backwards copy)*
    ...
    > 6. **Waived, 2026-09-15 (lead, reviewed): the weekly track does not carry the bonus, and that is correct.**
    >    The premise was wrong — M7.3's weekly rating is a second, independent read-time fold (`rateGameWeekly`
    >    over the week's own seeds in `lib/board/weekly.ts`), not a call to `foldGame`, and a `WeeklyPlayer` carries
    >    no stat line to score. Widening it would mean adding the nine performance columns to `/leaderboard`'s own
    >    `game_players` select — a second surface change M7.9 does not own. Confirmed with a test
    >    (`weekly.test.ts`) recording the absence rather than a special case. If the group wants the week to carry
    >    the bonus too, that is a new task, not owed here.
...
- [x] **M7.10** (landed 2026-09-16: the Discord result embed names `MVP <name> · ACE <name>` as a third, unnamed field with a zero-width-space name — last field, so it's the first `guardEmbed` sheds over 6000 characters — and `/p/[puuid]`'s `Recent games` rows print the word beside the delta (`.cn-game-award`, delta's own size, no badge/icon/colour), with the explanation sentence joining the M5.15 strip as a second sentence in the same paragraph. Both surfaces call the identical exported `gatedGameAward` in `apps/web/lib/ingest/fold.ts` off the identical widened select (nine stat columns + role, same query, no new read — acceptance 4) — one function, so the two cannot disagree, proven by a cross-surface integration test (`apps/web/app/award.integration.test.ts`) that reads the same stored game through both the service-key Discord path and the anon-key page path. Two parallel worktrees (platform-engineer for the embed/loader, web-engineer for the player page) both independently built the loader half by a lead dispatch-briefing mistake; the lead reconciled by keeping the platform half's `gatedGameAward`-based implementation and layering the web half's rendering/copy/tests on top unchanged, since the two independently-built `RecentGame.award` shapes matched exactly. Reviewed clean after the reconciliation, with one doc gap the reviewer caught and the designer then closed: `05-design.md`'s copy-table rows and its stale two-field embed spec.) Name the MVP and the ACE where the number moved: the result post and `/p/[puuid]`'s rating explanation, from the columns M7.7 and M7.9 already write. *(owner: `platform-engineer` for the embed and the loader, `web-engineer` for the player page; after M7.9)*
    ...
    >
    > The embed and both loaders. **The web half landed the same day** and the whole task is ticked above.
    >
...
- [x] **M7.11** (landed 2026-09-16, run by the user against the hosted `kustom` project. Dry run first: 44 games considered, 38 rated, 6 skipped (2 participant-count, 4 game-mode — the ARAM games M7.1 now excludes), would change 420 `game_players` rows and 25 `ratings` rows. Real run (`--force`, one game had landed in between so it picked up 45 considered / 39 rated on its own, not an exit-2 mid-run case): wrote 430 `game_players` rows, 25 `ratings` rows, 24 seeds stored for the first time (the first full-season rebuild since the stored-seed table, M5.7, landed — most players never had a locked-in seed row before this), 8 inferred-role pairs moved, biggest single move 10.780 mu on a game that was five players' first-ever rated game (explainable: first game carries the highest `sigma`, and therefore the biggest possible OpenSkill swing). Third run confirmed idempotent: wrote 0 rows, 0 new seeds, 0 roles moved, biggest move "none". The group was told the same day; see the announcement text below.) The one rebuild. After every M7 task has landed, run `pnpm --filter web rebuild-ratings` exactly once, confirm it is clean, and tell the group. *(owner: `platform-engineer`, with the lead; after M7.1, M7.3, M7.4, M7.6, M7.9 and M7.10 — **the last task in the milestone that changes a number**; M7.12 below is numbered after it but is a read, is not gated by it, and owes no rebuild)*

    > **The announcement (told the group 2026-09-16):** "Ratings got recalculated tonight. Two changes: ARAM games
    > don't count toward your rating anymore (they never should have), and carrying a game now counts for a bit
    > more while being the only one trying counts for a bit less. Nothing was reset — same history, just re-added
    > under the two new rules. Everyone's number moved once."
...
- [x] **M7.14** (platform half landed 2026-09-15, reviewed clean: migration `0015_damage_to_objectives.sql` on `game_players`, nullable, no default, `>= 0` check as defence in depth, `damage_to_objectives`/`damageToObjectives` named after its sibling `damage_to_champs` rather than the client's own words; `RawGameFacts` reads it uppercase-first with the load-bearing camelCase fallback (proven against a match-detail fixture carrying no uppercase key at all); ingest writes it through `storedStat` beside M7.7's two; `copyRawStats` extended to a third column with its own per-column shortfall count, so the objectives number can't hide behind M7.7's already-zero one. **Pushed to hosted and backfilled 2026-09-15** (the user): migration `0015` applied, `copy-raw-stats` ran against the hosted project — 44 games with a gap, all 44 fillable, 436 rows filled on `damage_to_objectives`, **0 rows still short on any of the three columns**. **Core half landed 2026-09-15, reviewed clean, every acceptance number independently recomputed by hand**: the seventh `damageToObjectives` component in `performanceScores`, the exact weights (jungle 0.20/0.20/0.10/0.15/0.10/0.10/0.15, carry and support unchanged at 0.00 on the seventh), the universal missing-input rule proven specifically against a *carry* player's null (not just a jungler's, the case a weight-scoped rule would get wrong), bit-for-bit-unchanged carry/support scores proven with exact (`toBe`) equality, and the "jungler who took the map" test showing a real MVP flip (six-component adc 0.5625 beats jungler 0.5000; seven-component jungler 0.5750 beats adc 0.5625). `00-product.md`'s "Carrying is worth a little more" updated to the seven-part reading. **The battle-test script fix and the jungle-only addendum both landed 2026-09-15**: `m7-13-battle-test.ts` reads the seven-component export and gained `--addendum`, run against a fresh hosted extraction (23 games, `damage_to_objectives` included) — **0 of 23 MVPs moved, 0 of 23 ACEs moved, all 184 carry/support seats bit-for-bit identical on real data**, and every jungle score's movement tracked its own objective share (the largest riser took 100% of the game's best objective damage, the largest faller 0.8%) — see the addendum block under this brief. **M7.14 is complete.**) Damage to objectives: the seventh component, and the one row that reads it. A migration, an
  ...
  core ✓. **It does not gate M7.9 or M7.10. It must land before M7.11**, the one rebuild.)*
    ...
    >
    > ### Addendum (platform-engineer + lead, 2026-09-15)
    >
    > Per M7.13's own amendment: M7.13's battle test already ran two columns (flat vs six-component) before this
    > task's core half landed, so this owes a jungle-only addendum rather than a second full run — six-component
    > bucket weights against the real seven-component export, over the same 23 real ten-human customs, run via
    > `pnpm --filter web m7-13-battle-test <rows.json> --addendum` against a fresh hosted extraction that includes
    > `damage_to_objectives`. Read-only; nothing was written.
    >
    > | # | night | jungler | side | obj dmg | obj share | six | seven | Δ | pick six | pick seven | moved |
    > |---|---|---|---|---|---|---|---|---|---|---|---|
    > | 1 | 09-08 22:39 | Ramzyinhović | lost | 35917 | 1.000 | 0.548 | 0.593 | +0.045 | | | |
    > | 1 | 09-08 22:39 | PRT Khokha | won | 30303 | 0.844 | 0.514 | 0.542 | +0.027 | | | |
    > | 2 | 09-08 23:25 | Ramzyinhović | won | 40676 | 1.000 | 0.893 | 0.893 | -0.000 | MVP | MVP | |
    > | 2 | 09-08 23:25 | Bot | lost | 23271 | 0.572 | 0.392 | 0.402 | +0.009 | | | |
    > | 3 | 09-09 21:56 | PRT Shou3man | won | 14557 | 0.551 | 0.438 | 0.438 | -0.000 | | | |
    > | 3 | 09-09 21:56 | 1sec Reloading | lost | 24475 | 0.926 | 0.540 | 0.579 | +0.039 | | | |
    > | 4 | 09-09 22:43 | Bot | lost | 14409 | 0.296 | 0.592 | 0.538 | -0.054 | | | |
    > | 4 | 09-09 22:43 | 1sec Reloading | won | 48651 | 1.000 | 0.554 | 0.615 | +0.061 | | | |
    > | 5 | 09-10 22:59 | Ramzyinhović | won | 41503 | 1.000 | 0.546 | 0.600 | +0.054 | | | |
    > | 5 | 09-10 22:59 | FoxHound | lost | 8573 | 0.207 | 0.398 | 0.365 | -0.033 | | | |
    > | 6 | 09-10 23:32 | SugarPapy | lost | 188 | 0.008 | 0.375 | 0.304 | -0.071 | | | |
    > | 6 | 09-10 23:32 | FoxHound | won | 24480 | 1.000 | 0.566 | 0.607 | +0.041 | | | |
    > | 7 | 09-11 00:05 | Ramzyinhović | lost | 19311 | 0.316 | 0.643 | 0.592 | -0.051 | | | |
    > | 7 | 09-11 00:05 | FoxHound | won | 61152 | 1.000 | 0.564 | 0.627 | +0.064 | | | |
    > | 8 | 09-11 21:29 | 1sec Reloading | lost | 5598 | 0.149 | 0.469 | 0.433 | -0.037 | | | |
    > | 8 | 09-11 21:29 | knifiy | won | 37498 | 1.000 | 0.803 | 0.820 | +0.017 | MVP | MVP | |
    > | 9 | 09-11 22:13 | Bot | won | 38566 | 0.999 | 0.583 | 0.613 | +0.030 | | | |
    > | 9 | 09-11 22:13 | knifiy | lost | 12301 | 0.319 | 0.502 | 0.454 | -0.048 | | | |
    > | 10 | 09-11 22:57 | FoxHound | won | 40591 | 1.000 | 0.452 | 0.509 | +0.057 | | | |
    > | 10 | 09-11 22:57 | knifiy | lost | 30890 | 0.761 | 0.521 | 0.547 | +0.027 | | | |
    > | 11 | 09-11 23:49 | knifiy | won | 36638 | 1.000 | 0.678 | 0.699 | +0.021 | | | |
    > | 11 | 09-11 23:49 | PRT Shou3man | lost | 4220 | 0.115 | 0.415 | 0.359 | -0.056 | | | |
    > | 12 | 09-12 21:24 | SugarPapy | won | 38694 | 1.000 | 0.430 | 0.498 | +0.068 | | | |
    > | 12 | 09-12 21:24 | PRT Khokha | lost | 18341 | 0.474 | 0.464 | 0.466 | +0.002 | | | |
    > | 13 | 09-12 22:03 | Ramzyinhović | won | 20627 | 0.641 | 0.572 | 0.555 | -0.017 | | | |
    > | 13 | 09-12 22:03 | SugarPapy | lost | 32157 | 1.000 | 0.385 | 0.455 | +0.070 | | | |
    > | 14 | 09-12 22:43 | PRT Shou3man | won | 37621 | 1.000 | 0.551 | 0.610 | +0.060 | | | |
    > | 14 | 09-12 22:43 | PRT Empty | lost | 30260 | 0.804 | 0.482 | 0.525 | +0.043 | | | |
    > | 15 | 09-12 23:36 | PRT Empty | won | 28665 | 0.795 | 0.539 | 0.572 | +0.032 | | | |
    > | 15 | 09-12 23:36 | PRT Shou3man | lost | 36061 | 1.000 | 0.722 | 0.746 | +0.024 | | | |
    > | 16 | 09-13 21:13 | knifiy | lost | 41271 | 0.857 | 0.446 | 0.494 | +0.048 | | | |
    > | 16 | 09-13 21:13 | PRT Empty | won | 48165 | 1.000 | 0.372 | 0.448 | +0.075 | | | |
    > | 17 | 09-13 22:30 | Raafat | won | 50889 | 1.000 | 0.541 | 0.597 | +0.056 | | | |
    > | 17 | 09-13 22:30 | knifiy | lost | 15125 | 0.297 | 0.546 | 0.504 | -0.042 | | | |
    > | 18 | 09-13 23:13 | knifiy | won | 41302 | 1.000 | 0.506 | 0.562 | +0.056 | | | |
    > | 18 | 09-13 23:13 | Ramzyinhović | lost | 2254 | 0.055 | 0.387 | 0.325 | -0.061 | | | |
    > | 19 | 09-13 23:43 | knifiy | lost | 6773 | 0.308 | 0.440 | 0.422 | -0.018 | | | |
    > | 19 | 09-13 23:43 | PRT Shou3man | won | 20584 | 0.935 | 0.677 | 0.694 | +0.017 | | | |
    > | 20 | 09-14 21:16 | FoxHound | lost | 8971 | 0.226 | 0.434 | 0.395 | -0.039 | | | |
    > | 20 | 09-14 21:16 | PRT Shou3man | won | 39712 | 1.000 | 0.509 | 0.567 | +0.058 | | | |
    > | 21 | 09-14 21:55 | PRT Shou3man | won | 23866 | 0.528 | 0.615 | 0.591 | -0.024 | | | |
    > | 21 | 09-14 21:55 | SugarPapy | lost | 36011 | 0.797 | 0.715 | 0.700 | -0.016 | | | |
    > | 22 | 09-14 22:46 | PRT Empty | won | 32559 | 0.892 | 0.657 | 0.683 | +0.025 | | | |
    > | 22 | 09-14 22:46 | PRT Toxicator | lost | 36516 | 1.000 | 0.334 | 0.425 | +0.091 | | | |
    > | 23 | 09-14 23:45 | Raafat | lost | 18784 | 0.480 | 0.438 | 0.418 | -0.019 | | | |
    > | 23 | 09-14 23:45 | SugarPapy | won | 39099 | 1.000 | 0.660 | 0.673 | +0.013 | | | |
    >
    > **Totals.** Games scored under both: 23 of 23. **MVPs that moved: 0 of 23. ACEs that moved: 0 of 23.**
    > Neither pick moved on any of the 23. Jungle seats: 46 — score rose on 29, fell on 17, identical on 0;
    > largest rise +0.091, largest fall -0.071. **Carry and support seats: 184 checked, every one bit-for-bit
    > identical under six and seven** — M7.14's central claim confirmed on real data, not only in synthetic tests.
    >
    > **Judgement.** The question this addendum asks is narrower than M7.13's own: not "are the picks better" but
    > "did giving a jungler 0.15 for objectives move the right junglers, and only junglers". It did. Every large
    > riser took most or all of the game's best objective damage (PRT Toxicator +0.091 at 100% share, PRT Empty
    > +0.075 at 100%), and every large faller took almost none (SugarPapy -0.071 at 0.8% share, Ramzyinhović -0.061
    > at 5.5%). No MVP or ACE actually changed hands in these 23 games — the seventh component moves jungle scores
...
- [x] **M7.16** (landed 2026-09-16, both halves by `web-engineer`: on `this-week` and `last-week` every rating
  number on `/p/[puuid]` is the weekly track's, off **the same fold the board runs**.
  **The loader.** `lib/board/load.ts` grew one `weeklyFold` helper — `weeklyGames` plus `seedFor`'s seed rule,
  handed to `foldWeeklyRatings` — and `windowRows` and `loadPlayerBoard` both call it, so there is exactly one
  `foldWeeklyRatings` call site in the file and `lib/board/weekly.ts` is still the only file in the app that
  imports `rateGameWeekly` (pinned by a grep test in `weekly.test.ts`, beside M7.3's existing `lib/ingest`
  guard). The page's own read is two extra queries on the two week windows only, made in parallel with
  `Recent games`: the week has to be read whole, because a player's weekly number is a property of the ten seats
  of every game in it. `PlayerBoardView` gains `track`, exactly `BoardRow.track`'s two values, and the view
  branches on it rather than on the window.
  **What changes on the page.** The one number is the weekly `Rating`, equal to the digit on that player's board
  row (integration-tested against `loadBoard` for *every* player on a fixture week, both week windows); **no
  Proven anywhere** — not a second number, not a label, not small type — and **no `settling` chip**, the board's
  own week carve-out; the chart is the weekly fold's series with the weekly seed under the existing `start`
  label (never `seed`); each `Recent games` delta for a game inside the window is the weekly one, and the row's
  expand on `/leaderboard` and the page's list are asserted equal game for game. The record is unchanged. A
  player with no counted game in the week reads their weekly seed and the window's empty line. `All time`,
  `This month` and `Last month` are byte-identical, `SETTLING_SENTENCE_PLAYER` included.
  **The copy.** New `WEEK_PLAYER_SENTENCE` in `lib/board/copy.ts`, printed once per page in `SettlingNote`'s
  slot on the two week windows (component `WeekPlayerNote`), third person because the page may be somebody
  else's (M3.26). **The brief's suggested wording was corrected before shipping**: it said `back at their rank
  on Sunday`, written hours before M7.19 landed, and nothing starts from a rank any more — the sentence reads
  `Every week starts everyone on the same rating on Sunday, …`. Pinned by code point in `board.test.ts` with
  guards for the retired word, the four rules and the third person.
  **One defect found and not fixed, because it is product's and is a different surface:**
  `WEEK_BOARD_SENTENCE` and `WEEK_BOARD_SENTENCE_SHORT` — `/leaderboard`'s week sentence and both week posts'
  footer — **still say `starts everyone back at their rank on Sunday` and are stale after M7.19** in exactly the
  way this task's brief was. Copy only, no number, no rebuild; it wants the same one-clause correction and a
  re-pin of the two strings in `board.test.ts` and `05-design.md`'s copy table. Not touched here rather than
  silently re-worded on another task's surface. **Opened as M7.21 the same day**, with both corrected strings
  written and the docs updated; the constants and their tests are a `web-engineer` pass.
  Two smaller rules settled in passing and recorded in the decision row: a game inside the week the weekly fold
  did not rate reads `not rated` on that window, and the `MVP` / `ACE` word never moves with the track (M7.10).)
...
- [x] **M7.17** (landed 2026-09-16: `NOT_RATED_HINT` in `apps/web/lib/board/copy.ts` is now
  `Some games don't move ratings: ARAM, too short, short a player, or added from match history and not counted yet.`
  — one word added, nothing else in the sentence touched, no second sentence and no argument for why ARAM does
  not rate. `NOT_RATED` and every other copy constant are untouched, and the placement rule is unchanged: once
  per page, only while a row reads `not rated`, which is `PlayerView.tsx`'s existing `unrated` guard and its two
  existing tests. Pinned by code point in `apps/web/lib/board/board.test.ts`, with a second guard that the new
  word appears in this string and in no other constant. `05-design.md`'s copy-table row and its "Rating history"
  prose carry the new string and the designer's M7-review defect against them is marked fixed; the tonight-page
  paragraph that pointed at that defect now points at the repaired sentence. No rating, no gate, no rebuild.)
  Say that ARAM is why. The one sentence under `Recent games` that explains an unrated game lists
...
- [x] **M7.18** (landed 2026-09-16, `web-engineer`: **no number, query, gate or loader changed** — words beside
  counts that already existed, and no rebuild owed. `boardSlotLine` and `ratedGamesLabel` are a **second
  formatter** in `apps/web/lib/board/copy.ts`; `windowSlotLine` is byte-for-byte unedited and keeps `/stats`,
  `/fun` and `/games`, which is pinned in `lib/board/counts.test.ts` and re-asserted in `StatsView.test.tsx`.
  `/leaderboard`'s slot now reads `Sunday 13 Sep to Saturday 19 Sep · 12 rated games` on all five windows
  (`· 1 rated game` at one, never `rated 1 game`, never `· 0 rated games` — the empty window keeps its sentence),
  pinned by code point in `counts.test.ts` and at the component level in `BoardView.test.tsx`. The **Sunday and
  monthly** board posts carry the same string because `post.ts` composes the description with the same function
  (`windowPost.test.ts`, four `window.integration.test.ts` cases, and the embed snapshot); **the nightly post has
  no slot line at all** — `leaderboardEmbed` takes no description, and its per-row `· 41 games` is the board
  row's own count and was left alone so the post and the page keep saying one thing. `/p/[puuid]` prints
  `PLAYER_COUNTS_SENTENCE` once, between the rating card and `By role`, and is absent on a window with no
  sections and for a player whose whole window was ARAM (no record above it to explain). **The header record
  carries no count and therefore no `rated` wording at all** — see the note below; acceptance 3's first clause
  is not met by this task.
  `lib/stats/fold.ts`'s header comment drops M7.1's retired promise and names `gateGame` and `gateRatedGame`.
  A fixture-window test pins the board's 3, `/stats`' 4 and `/p/[puuid]`'s 4 over one month holding an ARAM and a
  remake, so the wording cannot land beside a count that quietly moved. `05-design.md` carries four copy-table
  rows and the two prose passages; `04-decisions.md` carries the row. **Acceptance 3's first clause is NOT met,
  and the first landing said otherwise:** `/p/[puuid]`'s header record has printed **no count since M5.22** — the
  designer's rule, in `05-design.md`, is that the meta line drops its games count whenever the seed line is
  present, the seed line is present on every window that has a record, and the line is absent at zero games, so
  there is **no window and no player** on which `37 rated games · 20W 17L` can print. M7.18 first shipped a
  `start === null` branch for it anyway; the reviewer found it was unreachable for every window, every player and
  every dataset, and it was deleted on 2026-09-16 along with `PlayerView.tsx`'s `ratedGamesLabel` import (the
  formatter itself stays — `boardSlotLine` is its caller). **Only acceptance 3's second clause landed here**: the
  one sentence above the stats sections. The rated count a reader could actually see on the page was the **seed
  line's** `, 37 games since.` — `player.games`, worded as plain `games` directly above played-count sections —
  which was exactly the ambiguity the task was opened to fix. **M7.22 closed it on 2026-09-16** by naming the
  count in the seed line's own clause (`, 37 rated games since.`, `sinceClause` calling `ratedGamesLabel`), with
  M5.22's placement kept and the header record still carrying no count. **Acceptance 3 is therefore fully met as
  of M7.22**, in the one place on the page where a rated count prints.)
  One window, two counts, and nothing says which. `/leaderboard` counts the games that moved a
  ...
  them. *(owner: `web-engineer`, with the designer for placement; after M7.1 ✓ and M7.11 ✓)*
    ...
    >    **(Landed half-met 2026-09-16, and completed the same day by M7.22.** The second clause landed with this
    >    task. The first could not, as written: M5.22 leaves the header record with no count on any window, so
    >    there is nothing on that line to name, and the branch that pretended otherwise was unreachable code and
    >    has been removed. **M7.22 met it where the count actually prints** — the seed line, whose clause now
    >    reads `, 37 rated games since.` through `ratedGamesLabel`, with M5.22's placement kept rather than
    >    inverted. Read this check as "the rated count on `/p/[puuid]` is named, once, wherever it prints": it is,
    >    on all five windows, and **acceptance 3 is now fully met**.)**
...
- [x] **M7.19** (code half landed 2026-09-16, reviewed: `provisionalSeed()` in `packages/core`, the new
  `config.rating.provisionalSigma`, `seedFor`'s fallback, the two inline copies in `lib/ingest/rating.ts` folded
  into it, and every player-facing rating for a player with no `ratings` row. **The copy decision landed
  2026-09-16**: both sentences are written, in `05-design.md`'s copy table, in a decision row and in "Step 3
  settled" at the foot of this brief — `Started at 1200, 37 games since.` and `Everybody starts on the same
  rating, and every Summoner's Rift result moves it. Proven is the board's careful version of it and settles
  after about 30 games.` **The hosted re-seed and rebuild landed 2026-09-16** (the user, running the commands;
  the lead verifying): `update ratings set seed_mu = 20, seed_sigma = 12` on the hosted project — `seed_rank_tier`
  / `seed_rank_division` untouched, preserving the historical rank record, exactly as the corrected plan required
  — then `pnpm --filter web rebuild-ratings`. Dry run: 45 games considered, 39 rated, would change 390
  `game_players` rows and 25 `ratings` rows, 0 seeds to store for the first time (every player already had a
  seed row; the reset only changed its value, never recreated it — confirming the non-destructive approach
  worked). Real run: wrote the same 390/25, biggest single move 11.637 mu **down** on the same player M7.11's
  own biggest mover named, refolded from the new starting point — explainable the same way M7.11's was (early
  games are the most sensitive to where the fold starts, and this player's whole history was just refolded from
  a different one). Third run confirmed idempotent: 0 rows written. **The copy pass landed 2026-09-16**, the
  `web-engineer` one spelled out line by line below: `seededLine` lost its `rank` parameter and now returns
  `Started at 1200, 37 games since.` (`Started at 1200.` at zero games, `1 game` at one, still through
  `gamesLabel`), `HOW_THIS_WORKS_LINES[3]` is product's two-sentence string, `explainRatingStart` passes
  `player.reference` and `player.games`, and both strings are pinned by a new guard each — a `Silver II`-seeded
  fixture's all-time line names neither `Silver` nor `Seeded`, and `Shell.test.tsx` carries the twin of
  `board.test.ts`'s `promises a gap that settles` over the shell line. Acceptance 8 is ticked with it. No page,
  post or embed prints a rank beside a rating any more; `lib/board/load.ts`, `lib/tonight/`, `lib/ingest/` and
  `packages/` are untouched, which leaves `PlayerBoardView.seedRank` and `rankLabel` loaded and unread —
  **M7.20's question, deliberately not answered here**.) **Nobody's rating starts
  from their League rank any more.** Every stored rating — all-time and weekly — begins at one provisional seed,
  `{ mu: 20, sigma: 12 }`, the same for everybody, and the customs decide the rest. *(owner: the user's decision;
  `core-engineer` for the constant and the export ✓, `platform-engineer` for the seed rule and the pages ✓ and for
  the hosted re-seed and the rebuild, `product` for the one sentence, `web-engineer` for rendering it; after M7.11
  ✓ — **this is the second and last rebuild the milestone owes**)*

    > **Brief (the user, 2026-09-16; the sigma measured by the engineer. Full decision row: `04-decisions.md`,
    > 2026-09-16.)**
    >
    > ### The complaint
    >
    > Two people who play these customs equally well held ratings 1,260 display points apart because one of them
    > climbs in solo queue. That is what `seedFromRank` did to a first number, and M7.3 made it worse without
    > meaning to: the weekly fold reseeds from the same stored seed every Sunday, so a rank was not a fading first
    > guess, it was re-injected into a board that is supposed to measure *this week* for ever. On game one the seed
    > is the whole answer, and a Challenger at 2100 against an Iron at 840 is the model asserting something it has
    > no evidence for, on the board that forms the teams.
    >
    > ### The decision
    >
    > **`provisionalSeed()` = `{ mu: 20, sigma: 12 }`, no arguments, both tracks.** `mu` 20 says nothing about you.
    > `sigma` 12 is a **third** constant — deliberately larger than `rankedSigma` 8.33 and `unrankedSigma` 10,
    > which keep their own job — and it is the whole mechanism for "a new player swings hard for about five games,
    > then moves normally": OpenSkill moves `mu` in proportion to a player's own `sigma²`, and `sigma` shrinks
    > fastest while it is large, so one number in the seed produces the whole curve under the same untuned
    > `rateGame` for every player in every lobby. **No two-phase model, no placement flag, no `beta` / `tau`
    > change, no per-match special case.** 12 is measured and not chosen for sounding fast: a simulated newcomer of
    > known true skill against nine settled opponents is worst-case 5.59 mu from the truth after five games at
    > sigma 12, 7.92 at the old rank seed, and **9.60 at sigma 20** — past the knee the extra step buys win/loss
    > noise, not accuracy. The full table is in `config.ts` and in the decision row; the test keeps a bounded guard,
    > not a pinned number.
    >
    > **`seedFromRank` stays, with one caller.** `lib/ingest/balance.ts` needs *some* number for a face with zero
    > customs games so tonight's split is not a coin flip, and their solo-queue rank is the only thing anybody
    > knows. That guess lives for one evening and is never written down. Deleting it was refused for exactly that
    > reason.
    >
    > **`ratings.seed_rank_tier` / `seed_rank_division` stay too**, still written, still saying what the client
    > reported the night a history started. They are informational from here on and drive no number — which means
    > a seeded player's rank label and their seed number are now free to disagree, by design.
    >
    > ### Where the line falls between a rating surface and a lobby surface
    >
    > A **rating** surface shows what the model holds. `/leaderboard`'s rows and both numbers at the top of
    > `/p/[puuid]` therefore read 1200 for a player with no folded row, beside `0 games` and the `settling` chip,
    > agreeing with the reference line under the chart. Before this they read the balancer's rank guess while the
    > line under the same chart read the seed, so a never-rated Diamond's own page printed 1920 and 1200 forty
    > pixels apart.
    >
    > A **lobby** surface shows what tonight's split was formed from. `lib/tonight/load.ts` keeps the rank
    > estimate, because every seat number on that page is also in the Discord teams embed and the two are built
    > from one pool; "correcting" it would only make the page and the message about the same ten people disagree.
    > The seam is one evening wide and closes the moment that player's first game is folded.
    >
    > ### Step 2: the retroactive re-seed, and the way it must not be done
    >
    > Every stored seed in the hosted project was written under the old rule — the 24 M7.11 wrote on 2026-09-16
    > included — so the code change alone leaves the group's actual numbers folded from ranks. The fix is
    > **`update ratings set seed_mu = 20, seed_sigma = 12`** across the season, then one `rebuild-ratings` run,
    > M7.11's pattern: dry run, read the numbers back to the user, `--force` only if a game landed inside the
    > fifteen-minute guard, confirm idempotent on a third run, tell the group.
    >
    > **It is not "null every seed and re-run the rebuild", and that shortcut is a data loss.** `seedFor(null, …)`
    > does recompute to exactly the provisional pair, so the numbers would come out right — but `rebuild.ts` writes
    > a freshly derived seed through `seedColumns`, which also stores `seed_rank_tier` / `seed_rank_division` from
    > the player's **current** `players.rank_tier`. Nulling would therefore restamp every row with tonight's rank
    > and destroy the one thing those two columns exist to hold. Setting the two numbers directly leaves the record
    > standing, and the refold then reads the stored pair, so `sameSeed` keeps the rank columns out of the write.
    >
    > ### Step 3: the two sentences
    >
    > `/p/[puuid]` says `Seeded from Gold IV at 1200.` (M5.15). Both halves are true and the sentence implies a
    > causation that no longer exists. And the shell's `How this works`, on **every page of the site**, says
    > `Your rating starts from your rank and moves with every result.` (`lib/shellCopy.ts`) — which is now simply
    > false, and is the same claim as the first one in a louder place. **Both re-words are product's**, not an
    > engineer's; they are the only part of this task that is a copy decision, they should be written together,
    > and `05-design.md`'s copy table and its claim table both owe the row. Do not invent either in a loader.
    > The engineer's half of step 3 is already done: `05-design.md`'s claim table is marked false as written.
    >
    > ### What this task may not do
    >
    > Touch `rateGame`, `rateGameWeekly`, `beta`, `tau`, `seedFromRank`'s maths, or `lib/ingest/balance.ts`'s
    > caller. Add a placements phase, a second code path for new players, or a column to un-set. Drop the two
    > `seed_rank_*` columns. Run a rebuild before the re-seed.
    >
    > ### Acceptance
    >
    > 1. `provisionalSeed()` is exported from `packages/core`, takes no argument, returns
    >    `{ mu: config.rating.unrankedMu, sigma: config.rating.provisionalSigma }` = `{ 20, 12 }`, and a test pins
    >    that its sigma is greater than both `rankedSigma` and `unrankedSigma`. ✓
    > 2. `seedFor` is the one first-seed rule and its fallback is `provisionalSeed()`; no code under
    >    `apps/web/lib/ingest/` outside `balance.ts` calls `seedFromRank` at all. ✓
    > 3. The simulation guard in `rating/index.test.ts` is bounded, not pinned: worst error under 6.00 mu, at least
    >    1.5 mu better than the old rank seed, and sigma 20 measurably worse than the old rank seed. ✓
    > 4. No player-facing page shows a rank-derived rating: a never-rated player reads `1200`, `Proven 0`,
    >    `0 games` and the seed line's 1200 on `/leaderboard` and `/p/[puuid]`, pinned by an integration test that
    >    asserts against `provisionalSeed()` and not against a value that merely shares `unrankedMu`. ✓
    > 5. `lib/tonight/load.ts` and `lib/ingest/balance.ts` are unchanged and the Discord teams embed's numbers are
    >    byte-identical for the same pool (test). ✓
    > 6. The hosted re-seed runs as an `update` of the two numeric columns only; a query afterwards shows every
    >    `seed_mu` at 20 and **every `seed_rank_tier` unchanged from its 2026-09-16 value**, and the one
    >    `rebuild-ratings` run that follows is confirmed idempotent on a re-run.
    > 7. The group is told, in the M7.15 register: everybody starts at 1200 now, and your first few nights move you
    >    fast because we do not know you yet.
    > 8. `/p/[puuid]`'s seed sentence no longer implies the rank produced the number, and the shell's fourth
    >    `How this works` line no longer says a rating starts from a rank — both in product's words, both in
    >    `05-design.md`'s copy table before merge. ✓ **Words settled 2026-09-16** (copy table, decision row,
    >    "Step 3 settled" below) **and rendered the same day**: `seededLine(rating, games)` prints
    >    `Started at 1200, 37 games since.` with no rank clause and no `rank` parameter,
    >    `HOW_THIS_WORKS_LINES[3]` is product's two-sentence string, and both are pinned — the seed line by a
    >    guard that a `Silver II` fixture's sentence names neither `Silver` nor `Seeded`, the shell line by
    >    `Shell.test.tsx`'s twin of `board.test.ts`'s `promises a gap that settles`.
    > 9. `pnpm -r typecheck`, `pnpm -r test`, `pnpm lint` and `pnpm --filter web build` pass; `01-architecture.md`,
    >    `00-product.md` and the decision row say the same thing about where a rating starts. ✓
    >
    > ### Out of scope
    >
    > A placement-games phase or any second model for new players. Decay (M9). Re-tuning the weekly block. Any
    > change to what `seedFromRank` returns. A migration — this needs none.
    >
    > ### Step 3 settled (product, 2026-09-16): the two strings, and the pass that renders them
    >
    > Both sentences are decided. They are in `05-design.md`'s copy table and in a decision row of the same
    > date; what follows is the whole of the engineering pass, which is a `web-engineer` one and touches no
    > loader, no schema and no number. **Owner: `web-engineer`. No rebuild, no migration, no companion release.**
    >
    > **String 1, `/p/[puuid]`'s seed line** (`lib/board/copy.ts`, `seededLine`). The rank clause is **dropped,
    > not softened**:
    >
    > - `Started at 1200, 37 games since.`
    > - zero games: `Started at 1200.` (M5.15's rule is unchanged — no bare `0`)
    > - one game: `Started at 1200, 1 game since.` (still through `gamesLabel`)
    >
    > A rank printed on the one line that explains where a rating came from is read as the reason for it
    > whatever the preposition does, so `Diamond I at the time · started at 1200` was written out and refused.
    > The number stays `player.reference` — the chart's own hairline — so the sentence and the line cannot
    > disagree, and because it is interpolated the line is true both before the re-seed and after it. `All time`
    > now rhymes with the windows already under it: `Started at …` / `Started the week at …`.
    >
    > The signature loses its first parameter (`noUnusedParameters` is on, so it cannot merely go unused):
    > `seededLine(rating: number, games: number)`, and `explainRatingStart` calls it with `player.reference` and
    > `player.games`. **`player.seedRank`, `rankLabel` and the two `seed_rank_*` columns all stay where they
    > are** — the loader is unchanged in this pass, and whether a view field with no reader survives is M7.20's
    > question, not this one's.
    >
    > **String 2, the shell's fourth `How this works` line** (`lib/shellCopy.ts`), on every page of the site:
    >
    > - `Everybody starts on the same rating, and every Summoner's Rift result moves it. Proven is the board's
    >   careful version of it and settles after about 30 games.`
    >
    > One string, three falsehoods: the rank start (this task), `moves with every result` when an ARAM moves
    > nothing (M7.1), and `catches up`, the verb M3.19 retired on 2026-09-10 because the gap settles and never
    > closes. The designer's 2026-09-16 proposal fixed the last two and kept the first; it is superseded. Two
    > sentences and not three — the first-few-nights swing is real but the four lines are the whole system in
    > four lines, and the group hears that part in acceptance 7's own message. It does not name ARAM.
    >
    > **Every pinned test, by file** (these are all of them; `Shell.test.tsx` iterates the constant and needs no
    > edit for the string itself):
    >
    > - `lib/board/explain.test.ts` — `Started at ${player.reference}, 37 games since.`, `Started at
    >   ${player.reference}.`, `Started at ${player.reference}, 1 game since.`; the week test's
    >   `not.toContain('Seeded')` is now vacuous, so replace it with the assertion that the week line still
    >   reads `Started the week at 1469, 6 games since.` and fix the comment above it.
    > - `app/_board/PlayerView.test.tsx` — the two `Seeded from Silver II at …` expectations.
    > - `app/board.integration.test.ts` — `Started at ${player.reference}, 2 games since.`
    > - **New guard, one per string.** On the seed line: it never names a League rank — assert the all-time line
    >   for a `Silver II`-seeded fixture contains neither `Silver` nor `Seeded`. On the shell line, the twin of
    >   `board.test.ts`'s `promises a gap that settles` guard: `HOW_THIS_WORKS_LINES[3]` contains
    >   `Summoner's Rift` and `settles`, and contains neither `from your rank` nor `catches up`.
    > - Comments only, but they are the docs an engineer reads next and they name the retired sentence:
    >   `lib/board/copy.ts` (the `seededLine` and `rankLabel` docblocks, and the chart-legend note at ~line 259
    >   that still says the seed is "where the board started them from their rank"), `lib/board/explain.ts`
    >   (~line 58), `app/_board/PlayerView.tsx` (~lines 155 and 175), `lib/board/load.integration.test.ts`
    >   (~line 86). `apps/web/README.md`'s line about seeding from a rank *in memory* is still true — that is
    >   `balance.ts` — and stays.
    >
    > **Acceptance for the pass:** (a) the three seed-line forms and the shell line are byte-identical to the
    > strings above; (b) both guards are in and fail if the retired words come back; (c) `seededLine` takes two
    > arguments and no page, post or embed prints a rank beside a rating any more; (d) `pnpm -r typecheck`,
    > `pnpm --filter web test` and `pnpm lint` pass; (e) nothing under `lib/board/load.ts`, `lib/tonight/`,
    > `lib/ingest/` or `packages/` changes. When it merges, acceptance check 8 above is ticked — and only then,
    > because 8 is about what a player reads.

- [x] **M7.20** (landed 2026-09-16, `web-engineer` proposing and ruling, the choice being unambiguous once the
  callers were counted: **deleted, option (b)**. `PlayerBoardView.seedRank` is gone from `lib/board/types.ts`,
  from both branches of `loadPlayerBoard` in `lib/board/load.ts` and from `workedPlayer` in
  `lib/testing/boardFixtures.ts`; `rankLabel` and `UNRANKED_LABEL` are gone from `lib/board/copy.ts` with
  `copy.ts`'s now-unused `config` / `seedFromRank` import, and the seven `describe('the rank, as words')` cases
  are gone from `lib/board/explain.test.ts`. **A repo-wide grep before the call found no other caller**: the two
  loader lines and the one fixture line were `rankLabel`'s only production uses, `UNRANKED_LABEL`'s only reader
  was `rankLabel` itself, and `/admin/players` formats a rank with its own inline helper that was never this one.
  **`UNRANKED_LABEL` going too is the one thing beyond the brief's list**, by the same argument and noted here
  rather than done quietly. **The two `seed_rank_*` columns, `seedFor`, `seedFromRank`, `lib/ingest/seed.ts` and
  every rebuild path are untouched** — a new seed still stores the rank the client reported. Three comment blocks
  stand where the deletions were, in `copy.ts`, `types.ts` and `explain.test.ts`, so the next reader of M5.15's
  design learns the field was removed rather than mislaid. `load.integration.test.ts` keeps the 2026-09-11
  decision pinned through `reference` alone — the stored Gold II seed's number against a player wearing Diamond I
  — and gained a negative assertion that fails the day the loader reads tonight's rank; both its cases ran
  against the local stack, not skipped. No player-facing string changed, no page renders differently, no rating,
  fold, rebuild or migration. `pnpm -r typecheck`, `pnpm --filter web test` (112 files, 1584 tests),
  `pnpm lint` and `pnpm --filter web build` all pass.)
  The rank the board loads and nobody reads. M7.19's re-word takes the rank off `/p/[puuid]`'s
  seed line, which leaves `PlayerBoardView.seedRank` with no reader: `lib/board/load.ts` still builds it out of
  `rankLabel(seed.rankTier, seed.rankDivision)` in two places, `lib/board/types.ts` still declares it, the
  fixtures still set it, and `load.integration.test.ts` still pins that the loader reads the **stored** seed's
  rank rather than tonight's (the 2026-09-11 decision). Nothing typechecks wrong and nothing is displayed.
  Somebody decides, once: carry the field with a comment saying why a loaded-and-unprinted field is deliberate,
  or delete `seedRank` from the view type, the loader and the fixtures — which takes `rankLabel` and its seven
  unit tests with it, since after M7.19 those two `load.ts` lines are its only production callers.
  *(owner: `web-engineer` to propose, `product` to rule; after M7.19's copy pass; words and dead code,
  no rating, no rebuild, no migration)*

    > **No player sees anything change either way**, which is the whole point of keeping it out of M7.19: that
    > task's pass is two strings and their tests, and a type change dragged into it would be reviewed as part of
    > a copy fix. The two `ratings.seed_rank_*` columns are **not** in scope and are not dropped — they are the
    > record of what the client reported the night a history began, they are still written on every new seed,
    > and no page has ever been their justification. **Acceptance:** `seedRank` is either gone from
    > `lib/board/types.ts`, `lib/board/load.ts` and `lib/testing/boardFixtures.ts` — `rankLabel` and its unit
    > tests going too, since nothing else calls it — or it is still there under a comment that names this task
    > and says why a loaded-and-unprinted field is deliberate; no
    > player-facing string changes; `pnpm -r typecheck`, `pnpm --filter web test` and `pnpm lint` pass.

- [x] **M7.21** (landed 2026-09-16, `web-engineer`: `WEEK_BOARD_SENTENCE` and `WEEK_BOARD_SENTENCE_SHORT` in
  `apps/web/lib/board/copy.ts` now open `Every week starts everyone on the same rating on Sunday`, product's two
  strings character for character, with `Rating` still interpolated from `RATING_LABEL`. One clause out of each
  and nothing else: `board.test.ts` pins both full strings **and** asserts that putting `back at their rank`
  back reproduces M7.3's retired string byte for byte, which is the check that a rewrite did not happen
  (acceptance 2). Both doc comments gained product's `M7.19 took the League rank out of every stored seed`
  sentence, and `WEEK_PLAYER_SENTENCE`'s lost the "still carry the retired wording" clause for a note that all
  three now say the same thing with the M3.26 no-editing-one-into-another rule standing. The deliberately
  one-sided `rank` guard is a loop over all three week strings and its one-sidedness comment is gone. Every
  other test that reads these constants by import — `lib/discord/embeds.test.ts`, `lib/discord/windowPost.test.ts`,
  `app/_board/BoardView.test.tsx`, `app/_board/PlayerView.test.tsx`, `app/weekBoard.integration.test.ts` — passed
  unedited. Two files changed; no loader, no view, no rank read, nothing under `packages/`. No rating, no fold,
  no rebuild. Already-sent embeds keep the old footer on purpose; the next nightly post carries the new words.)
  The week board and both week posts still say a week starts you at your rank. `WEEK_BOARD_SENTENCE`
  and `WEEK_BOARD_SENTENCE_SHORT` in `apps/web/lib/board/copy.ts` both open `Every week starts everyone back at
  their rank on Sunday`, which M7.19 made false the same day it landed: no rating, weekly or all-time, is seeded
  from a League rank any more — everybody's week restarts on the same provisional number. The strings print on
  three live surfaces (the `This week` / `Last week` board sentence, the nightly post's footer and the Sunday
  post's footer), so this is the last place in the product that still tells a friend their rank starts their week.
  M7.16 found it, corrected the same clause in its own new `WEEK_PLAYER_SENTENCE`, and logged this rather than
  re-wording another surface's copy inside a page task. Product wrote the two corrected strings below; the pass
  is those two constants, their doc comments, and the tests that pin them.
  *(owner: `product` (the words, written 2026-09-16), then `web-engineer` (the constants and the tests); after
  M7.19; copy only — no rating, no fold, no rebuild, no migration, no companion release)*

    > **Brief (product, 2026-09-16)**
    >
    > ### What a friend reads now, and why it is wrong
    >
    > They open `/leaderboard` on `This week` — the default window, so this is the sentence most of the group
    > meets — and it says their week started them **back at their rank**. It did not. Since M7.19 every stored
    > seed is `provisionalSeed()`, the same `{ mu: 20, sigma: 12 }` for the Master player and the Bronze one, and
    > the weekly fold reads that same seed every Sunday. The Bronze player reading this sentence is being told the
    > board handicapped them in their favour and the Master one that it started them ahead; neither happened, and
    > the correction is the one M7.19 already made everywhere else it was said.
    >
    > ### The rule
    >
    > **One clause changes in each string and nothing else does.** `back at their rank` becomes `on the same
    > rating`, which is the substitution `WEEK_PLAYER_SENTENCE` already settled on 2026-09-16 — the point is that
    > everyone starts on the **same** number, not that they start on their own. Every other word, comma and em
    > dash in both sentences stays where it is: they were written and reviewed for M7.3 and only one fact in them
    > went stale. This is a correction, not a rewrite, and a diff that improves a second clause has gone wrong.
    >
    > **Both stay in the second person.** `WEEK_PLAYER_SENTENCE` is third-person because `/p/[puuid]` may be
    > somebody else's page (the M3.26 rule); a board is not, and neither is a Discord post — every reader of both
    > is in the group being described. The long string keeps `what the bot thinks you are`, and neither string
    > borrows the player page's `their rating` construction.
    >
    ...
    >
    > `Rating` in the long one stays interpolated from `RATING_LABEL`, exactly as it is today — the label and the
    > sentence may not drift, and the string is still assembled, not typed.
    >
    > ### The doc comments, which are part of the pass
    >
    > Each constant's comment gains the sentence that says why the clause reads this way, in the shape
    > `WEEK_PLAYER_SENTENCE`'s already does: **M7.19 (2026-09-16) took the League rank out of every stored seed —
    > `provisionalSeed()`, the same number for everybody — so a week restarts everyone on one rating rather than
    > on their own.** And `WEEK_PLAYER_SENTENCE`'s own comment loses the clause that says these two "still carry
    > the retired wording and are product's to fix (logged 2026-09-16)": it becomes a note that all three now say
    > the same thing, with the M3.26 rule that none may be edited into another left standing.
    >
    > ### Edge cases
    >
    > - **`Last week` prints the same two strings.** `Every week`, not `This week` — the M7.3 rule, unchanged, and
    >   the reason the opening clause is general in the first place.
    > - **An empty week still prints the sentence.** The board shows `No games this week yet.` above it, and the
    >   sentence is about how the week works, not about what is in it.
    > - **The monthly post and the month windows are untouched.** `SETTLING_SENTENCE`, `SETTLING_SENTENCE_SHORT`
    >   and `SETTLING_SENTENCE_PLAYER` print byte for byte, the `settling` chip and `SETTLING_GAMES` do not move,
    >   and no week string gains a `Proven` or a number.
    > - **`WEEK_PLAYER_SENTENCE` keeps its own words.** The board sentence is not copied onto the player page and
    >   the player sentence is not copied onto the board; after this pass the three differ only where they must
    >   (the sort clause, the person).
    > - **Nothing reads a rank to render any of this.** No loader, no view field and no `rankLabel` call is added
    >   or removed here — that is M7.20's question and it stays there.
    >
    > ### Acceptance
    >
    > 1. `WEEK_BOARD_SENTENCE` equals the long string above character for character, with `Rating` interpolated
    >    from `RATING_LABEL`; `WEEK_BOARD_SENTENCE_SHORT` equals the short one.
    > 2. Diffing each new string against its old one changes exactly the substring `back at their rank` →
    >    `on the same rating`: `WEEK_BOARD_SENTENCE.replace('on the same rating', 'back at their rank')` is the
    >    retired string, and the same for the short form. (Pin this, not just the full strings — it is the check
    >    that says a rewrite did not happen.)
    > 3. No string in `lib/board/copy.ts` contains `rank` in a player-facing sentence: the existing one-sided
    >    guard in `board.test.ts` (`WEEK_PLAYER_SENTENCE.toLowerCase()).not.toContain('rank')`) becomes a loop
    >    over all three week strings, and the comment explaining why it is one-sided goes with it.
    > 4. The three existing rules still hold on both strings: each starts `Every week`, contains neither
    >    `This week` nor `settles after` nor `Proven` nor any digit, and contains `All time is the settled one`;
    >    `WEEK_BOARD_SENTENCE` still contains `sorts on Rating`.
    > 5. `WEEK_PLAYER_SENTENCE`, all three `SETTLING_SENTENCE*`, `SETTLING_CHIP`, `SETTLING_GAMES` and
    >    `NOT_RATED_HINT` are byte-identical to before this task.
    > 6. The pinned expectations in `lib/board/board.test.ts` (the two `toBe` blocks under "the copy a week board
    >    says instead") are updated to the new strings, and `lib/discord/embeds.test.ts`,
    >    `lib/discord/windowPost.test.ts`, `app/_board/BoardView.test.tsx` and `app/weekBoard.integration.test.ts`
    >    still pass unedited — they compare against the constant, which is how it should stay.
    > 7. `pnpm -r typecheck`, `pnpm --filter web test` and `pnpm lint` pass. Nothing under `lib/board/load.ts`,
    >    `lib/board/weekly.ts`, `lib/ingest/`, `lib/tonight/` or `packages/` changes.
    > 8. `05-design.md`'s copy table and its `This week · leaderboard` embed mock carry the new strings (done by
    >    product with this brief, 2026-09-16 — the engineer checks it rather than writes it), and
    >    `04-decisions.md` has the row.
    >
    > ### Out of scope
    >
    > Any other sentence on any other window. Explaining `provisionalSeed`, `1200`, or why the rank is gone —
    > the shell's `How this works` lines already answer that site-wide (M7.19) and a week sentence is not where a
    > friend goes for it. Naming M7.19 or a date in player-facing text. Touching the two `ratings.seed_rank_*`
    > columns, `rankLabel` or `PlayerBoardView.seedRank` (M7.20). Re-posting a corrected footer to a Discord
    > message that is already sent: the next nightly post carries the new words and no backfill of old embeds is
    > owed.

- [x] **M7.22** (landed 2026-09-16, `web-engineer`: **one line of code**, option (b) exactly as product wrote it.
  `sinceClause` in `apps/web/lib/board/copy.ts` calls `ratedGamesLabel` instead of `gamesLabel`, so `/p/[puuid]`'s
  seed line reads `Started at 1200, 37 rated games since.` on `All time`, `Started the week at 1469, 6 rated games
  since.` on both week windows, `Started the month at 1469, 14 rated games since.` on both month windows, and
  `, 1 rated game since.` at one game. **The `games === 0` arm is byte for byte unchanged** — the clause is still
  dropped whole (`Started at 1200.`), and a window with no counted game still prints no line at all, because
  `explainRatingStart` returns `null` there. **`gamesLabel` is not edited and not re-pointed**: `windowSlotLine`
  still calls it and `/stats`, `/fun`, `/games` and `/leaderboard` are byte-identical, which `counts.test.ts`
  already pins and the full suite re-confirms. **`PlayerView.tsx`'s logic and imports are untouched** — no
  `ratedGamesLabel` import came back to the meta line, which still prints `20W 17L` and no count on every window —
  and `PLAYER_COUNTS_SENTENCE` is untouched at the seam. `seededLine`, `startedLine` and `explainRatingStart` keep
  their signatures; no caller changed. **Tests**: the five window forms pinned by code point in
  `lib/board/explain.test.ts` (all-time, week, month, one-game, zero-game) plus a loop over all five windows
  asserting the retired plain wording never returns; component-level in `app/_board/PlayerView.test.tsx`, a new
  five-window render loop and a zero-games guard beside the four existing seed-line assertions, all updated to the
  new string; and `app/board.integration.test.ts`'s pinned `Started at …, 2 rated games since.` against the local
  stack. Stale doc comments naming the old string were corrected in `copy.ts`, `explain.ts`, `types.ts`,
  `load.integration.test.ts` and `PlayerView.tsx`'s two comment blocks — **comments only in the component; no JSX,
  no logic, no import moved**. `05-design.md` and `04-decisions.md` were written by product before the code and
  were verified to match what shipped; no new decision row was owed. Copy only: no rating, fold, loader, query,
  gate or migration, and **no rebuild**.)
  The one count on `/p/[puuid]` that still does not say what it counted. M7.18 named the board's
  count, named `/stats`' count, and added the sentence at the seam — but its acceptance 3 assumed the player
  page's header record prints a count, and since M5.22 it does not on any window, for any player. The number a
  reader actually meets there is the **seed line's**: `Started at 1200, 37 games since.`, where `37` is
  `player.games`, the *rated* count, worded as plain `games` and sitting directly above `By role`, the streaks
  and the partners, every one of which counts games **played**, ARAM included. That is the exact ambiguity M7.18
  was opened to remove, on the one page where the two counts are forty pixels apart. Two ways to fix it, below;
  both edit a string product wrote, so product picks the words and the designer signs off the placement.
  *(owner: ~~`product` (the word choice), then `designer` (placement, if option (c))~~ → `web-engineer`; after
  M7.18 ✓ and M7.19 ✓; **product decided 2026-09-16: option (b)** — the designer is not needed, because (b)
  keeps M5.22's placement; copy only: no rating, no fold, no loader, no query, no rebuild, no migration)*

    > **Why an engineer may not just pick one** (the reviewer, 2026-09-16)
    >
    > Both options rewrite a sentence that is pinned by code point in `05-design.md`'s copy table and in tests,
    > and one of them was re-worded by **M7.19 eleven hours earlier** after an explicit product decision about
    > what that line may say. M7.19's rule for it was "the clause is dropped, not reworded"; adding a word back
    > into the same clause is the same kind of decision and goes the same way it did. The other option inverts
    > **M5.22**, which is a designer rule about which of two lines carries a number — also not an engineer's to
    > flip. So this task is scoped, not implemented, and the previous pass deleting its unreachable branch changed
    > nothing a reader sees.
    >
    > ### Option (b) — the seed line's own clause names its universe
    >
    > `, 37 games since.` becomes `, 37 rated games since.` on every window: `Started at 1200, 37 rated games
    > since.`, `Started the week at 1469, 6 rated games since.`, `Started the month at 1469, 14 rated games
    > since.`, and `, 1 rated game since.` at one.
    >
    > **What this touches, precisely.** `sinceClause` in `apps/web/lib/board/copy.ts` is private to that file and
    > has exactly two callers, `seededLine` and `startedLine`, which render exactly one thing: the seed line on
    > `/p/[puuid]`. Both pass `player.games`, the rated count. So **there is no non-rated context sharing this
    > clause** and the wording can move without a second formatter. What it must *not* touch is `gamesLabel`,
    > which `sinceClause` currently calls and which `windowSlotLine` also calls for `/stats`, `/fun` and `/games`
    > — those count played games and M7.18's acceptance 2 pins them byte for byte. The change is therefore
    > `sinceClause` calling `ratedGamesLabel` instead of `gamesLabel`; `gamesLabel` itself is unedited.
    >
    > **Cost:** the line gets longer and less plain, on the page where M5.15 deliberately made it prose. Product
    > may judge `37 rated games since` as jargon in a sentence that is supposed to read like a person talking.
    >
    > ### Option (c) — invert M5.22, and let the record carry the count
    >
    > The meta line under the two numbers goes back to carrying the count, named: `37 rated games · 20W 17L`,
    > and the **seed line drops its own count clause** instead — `Started at 1200.` / `Started the week at 1469.`
    > — so the page still prints the number once. This is M5.22's rule turned around rather than broken: the
    > designer's rule is "one count, one place", and this moves the place.
    >
    > **What this touches:** `sinceClause` becomes unconditional `'.'` (or is deleted with the branch), and
    > `PlayerView.tsx`'s meta line gains back the `ratedGamesLabel` span M7.18 wrote and 2026-09-16 removed as
    > unreachable — this time reachable, for every player with a rated game. `05-design.md`'s "the meta line
    > above it drops its games count when the seed line is present" passage is rewritten, not annotated.
    >
    > **Cost:** the designer's 2026-09-10 reasoning for M5.22 was that "since when" is what the count is *about*,
    > which is an argument for the seed line and against this. Reversing it needs the designer, not a note.
    >
    > ### Edge cases either option must answer
    >
    > - **Zero rated games** (a window somebody spent entirely on ARAM, or a brand-new player): no meta line
    >   today, and the seed line already drops its clause — `Started at 1200.` Neither option may print `0 rated
    >   games` anywhere, which is M5.15's acceptance 5 and still stands.
    > - **One game**: `1 rated game`, never `1 rated games` and never `rated 1 game` — `ratedGamesLabel`'s
    >   existing rule, already pinned in `lib/board/counts.test.ts`.
    > - **A window with no ARAM in it**: the word still prints. M7.18's "the label is not conditional" rule
    >   applies to this line for the same reason it applies to the board's slot.
    > - **The week windows** (M7.16): the count there is the weekly fold's rated count and the wording is the
    >   same; no second string for a week.
    > - **`PLAYER_COUNTS_SENTENCE` stays either way.** It explains the *seam*, not one line, and its placement
    >   rules (absent with no sections, absent with no record) do not move.
    >
    > ### Acceptance
    >
    > 1. Product has picked (b) or (c) and written the exact string(s); they are in `05-design.md`'s copy table
    >    before the code lands, and the copy-table row that currently reads "the header record on `/p/[puuid]`"
    >    is rewritten to describe what actually prints.
    > 2. On every one of the five windows, the rated count a reader sees on `/p/[puuid]` is named as rated,
    >    exactly once on the page — pinned by code point in `lib/board/copy.ts`'s tests and at the component
    >    level in `app/_board/PlayerView.test.tsx`.
    > 3. `gamesLabel`, `windowSlotLine` and every string on `/stats`, `/fun`, `/games` and `/leaderboard` are
    >    byte-identical to before this task (test). No count changes value anywhere.
    > 4. No unreachable branch: whichever line loses its count loses it unconditionally, and a test covers the
    >    zero-games and one-game cases on both lines.
    > 5. `docs/02-milestones.md` marks M7.18's acceptance 3 as completed by this task, and `04-decisions.md` has
    >    a row for whichever of M5.22 / M7.19's wording rule was amended.
    > 6. `pnpm -r typecheck`, `pnpm -r test`, `pnpm lint` and `pnpm --filter web build` pass.
    >
    > ### Out of scope
    >
    > Changing which games the header record counts, or the sections under it. A second count on the page. A
    > rated-only streak (M7.18's out-of-scope list still stands). Touching `ratedGamesLabel`'s own wording, the
    > board's slot line, or any Discord string. Re-opening whether the seed line names a rank (M7.19, settled).

    > ## Decision: **option (b)** — the seed line's own clause names its universe (product, 2026-09-16)
    >
    > `sinceClause` in `apps/web/lib/board/copy.ts` calls `ratedGamesLabel` instead of `gamesLabel`. One word,
    > one line, one file. **The designer is not needed**: M5.22's rule — the count lives in the seed line, the
    > meta line drops it — is kept exactly as written, so nothing about placement is reopened and no designer
    > sign-off is owed before an engineer starts.
    >
    > ### The exact strings, all five windows
    >
    > Verified 2026-09-16 against the current `sinceClause` / `seededLine` / `startedLine` and against
    > `explain.ts`'s `explainRatingStart`, which is their only caller:
    >
    > | Window | String |
    > |---|---|
    > | `All time` | `Started at 1200, 37 rated games since.` |
    > | `This week`, `Last week` | `Started the week at 1469, 6 rated games since.` |
    > | `This month`, `Last month` | `Started the month at 1469, 14 rated games since.` |
    > | any window, one game | `…, 1 rated game since.` — e.g. `Started at 1200, 1 rated game since.` |
    > | `All time`, zero games | `Started at 1200.` — **unchanged**, the clause is still dropped whole |
    > | a window with zero counted games | **no line at all** — unchanged, `explainRatingStart` returns `null` |
    >
    > The rating in each is `player.reference` (the chart's hairline) and the count is `player.games`, both
    > already passed in; the numbers above are the doc's standing examples and are not asserted by this task.
    >
    > ### Why the word is true on every one of the five
    >
    > Checked in `lib/board/load.ts`, not assumed: `player.games` is `stored.games` on `All time` (the `ratings`
    > row the fold writes) and `played.length` in a window, where `played` is `all.filter(row.muAfter !== null)`.
    > Both are the rated universe, on the week windows too. There is no window on which this line prints a
    > played count, so the adjective is never a lie and never needs a branch.
    >
    > ### Why (b) and not (c)
    >
    > 1. **The count is part of the sentence's meaning, not decoration on it.** `37 rated games since` answers
    >    *since when* — the second half of "where did this number come from". Moved onto the meta line it becomes
    >    `37 rated games · 20W 17L`, a count with no "since" in it, and the reader has to get that from the window
    >    heading. That is the designer's own 2026-09-10 argument for M5.22 and a year of it has not weakened.
    > 2. **`rated` is already this product's word, on this reader's other screens.** `/leaderboard`'s slot line,
    >    both board posts and the per-row `not rated` label all say it since M7.18, and the shell's fourth line
    >    says only Summoner's Rift results move the number. A friend arriving on `/p/[puuid]` has met the word
    >    already; it is one adjective, not a term of art, and the sentence still reads like a person talking.
    >    The jargon cost (b) carries is real and is smaller than the ambiguity it removes.
    > 3. **One formatter, so the two surfaces cannot drift.** The board's slot and this line both go through
    >    `ratedGamesLabel`, which is what stops one page saying `rated games` and the other inventing a synonym.
    > 4. **(c) reverses a designer decision to fix a wording problem**, needs a second owner before an engineer
    >    can start, rewrites a passage of `05-design.md` rather than amending it, and re-lands the very branch
    >    2026-09-16 deleted as unreachable. More moving parts, more doc churn, one owner more, for a page that
    >    would then say less.
    >
    > **What (b) gives up, honestly:** the seed line is two characters from prose and is now six longer. M5.15
    > wrote it as prose on purpose and this makes it very slightly more clinical. Product's judgement is that a
    > count a reader can misread as "every game I played" is worse than a sentence that is one adjective drier.
    >
    > ### What an engineer does
    >
    > - `sinceClause` returns `` `, ${ratedGamesLabel(games)} since.` `` and keeps its `games === 0 ? '.'` arm
    >   byte for byte. **`gamesLabel` is not edited and not re-pointed** — `windowSlotLine` still calls it and
    >   M7.18's acceptance 2 pins `/stats`, `/fun` and `/games`.
    > - `seededLine`, `startedLine` and `explainRatingStart` keep their signatures. No caller changes.
    > - Doc comments on `sinceClause`, `seededLine` and `startedLine` carry the new example strings; the M7.19
    >   note on `seededLine` stays as it is (this adds a word to the count clause and does not touch the rank
    >   clause M7.19 removed).
    > - `PlayerView.tsx` is **not** edited: no `ratedGamesLabel` import returns to the meta line, which still
    >   prints `20W 17L` and no count on every window.
    > - `PLAYER_COUNTS_SENTENCE` is untouched and still prints at the seam. Two things saying `rated` in two
    >   registers forty pixels apart is the intended outcome, not a duplication: one names a number, the other
    >   explains a seam.
    >
    > ### Acceptance, as decided
    >
    > The six checks above stand, read with (b) in them:
    >
    > 1. Done by this block, and `05-design.md`'s copy table carries the strings — the `the header record on
    >    /p/[puuid]` row and both seed-line rows were rewritten 2026-09-16, before the code.
    > 2. On each of the five windows the page's one rated count reads `rated game(s)`, pinned by code point in
    >    `lib/board/copy.ts`'s tests (all-time, week, month, one-game, zero-game) and at component level in
    >    `app/_board/PlayerView.test.tsx`.
    > 3. `gamesLabel`, `windowSlotLine` and every `/stats`, `/fun`, `/games`, `/leaderboard` string byte-identical
    >    (test). No count changes value.
    > 4. No unreachable branch: the meta line loses nothing because it has nothing to lose, and the zero-games and
    >    one-game cases are covered on the seed line.
    > 5. M7.18's acceptance 3 is marked completed by this task, and `04-decisions.md` has the row (added
    >    2026-09-16) recording that M7.19's "the clause is dropped, not reworded" rule governed the rank clause
    >    and does not forbid naming the count, and that M5.22 is kept rather than amended.
    > 6. `pnpm -r typecheck`, `pnpm -r test`, `pnpm lint`, `pnpm --filter web build`.
...
what, nobody wins it at all. And nobody's rating starts from their League rank: everybody's first number is the
same 1200, wherever they sit in solo queue, and their first few nights move it fast because the model has not
watched them play yet.
...
- [x] **M8.1** (landed 2026-09-15: a new "Friends and enemies" group on `/fun`. `Best duo` calls `duoRecords` verbatim — no second pair fold. `Nemesis` is new: a per-player, asymmetric fold over opposing-side games, ranked by losses against then worse record then more games then name, floored at `MIN_DUO_GAMES` and reused, never a second minimum. Every row prints its denominator, `Lost 7 of 9 to Lena.`, so the count cannot read as an attendance award; a perfect record against somebody is never a nemesis. Both lists expand into their customs newest-first and respect the `?queue=` split. Review confirmed the asymmetry, the tie-break order, and that `/stats`, the awards, the board and `packages/core` are untouched.) Nemesis and best duo on `/fun`. Who beats you, and who you win with. Query-only, over counted games in the open window. **Does not change `/stats`, the awards, or the rating model.** *(owner: `web-engineer`; after M7.4)*
    ...
    > its own task if the group asks twice. A head-to-head page (**opened as M8.5** when the group asked twice).
    > Champion-level rivalries. Any change to `duoRecords`, `compareDuos` or `compareDuosWorst`. A fourth award
    > (M8.3).
...
- [x] **M8.4** (both halves landed 2026-09-16: migration `0016` — `kind` with a two-value check, the `category` check re-added per kind, the unique moved to `(kind, challenge_number)` — plus `kindForDay` and `pickAwardStandout` in `select.ts`, the pure `build.ts` that builds either kind and falls back to a Daily Mystery when an award day has nothing scorable, `ensureTodayMystery` still the only writer, and `kind` on the play and result views for the card to read. The standout is `performanceScores` from `packages/core`; no scoring function was added under `lib/mystery/`. ARAM is out of the award selector. The view reused the existing card, not a second tree: `gameCopy(kind)` bundles the four sentences that differ, the closed card now names the award and prints its own number (`awardStatNumber` — an unstored M7.7/M7.14 column reads `Not recorded`, never `0`), and the countdown label and the nav tab are both kind-neutral (`Next game`, `Daily`) since neither can know or should pay to know today's kind. Reviewed once (three real defects: the closed card silently omitted the award's own number on four of seven categories, the countdown promised the wrong game under alternation, the nav tab named only one game); all three fixed and re-verified. **Migration `0016` pushed to hosted 2026-09-16** (`pnpm db:migrate`, confirmed with `supabase migration list --linked`; `pnpm db:types` regenerated with no diff). **`05-design.md` caught up 2026-09-16** (designer): the `Daily Mystery (M5.32)` section became `The daily game — Daily Mystery and Guess the Award (M5.32, extended by M8.4)` and now describes two games alternating civil days; the header sketch, the Nav bullet and the copy table's `nav` row all say `Daily` with the reason recorded; the bullets for the kicker, the distribution heading, the share lines and the countdown carry both readings; and a full two-column copy table plus a seven-row award table (`categoryLabel`, `awardStatLabel`, number format, which migration the column arrived in) were written from the shipped constants, with an award day drawn open and closed. Three layout flags recorded there, none a defect and none acted on: the closed kicker's longest pair wraps at 390 (a one-property `white-space: nowrap` fix if ever wanted), a `Widest KDA` day prints the KDA twice in two notations, and the play card's display cut is the KDA on both games, so on five of seven awards the biggest number is not the award's.) Guess the Award: a second daily game on the Daily Mystery engine, where the clues describe a standout stat line and the answer is who it belongs to. **The two games alternate days** (the user, 2026-09-15). Needs M7.7, M7.8 and M7.9. *(owner: `platform-engineer` for the service and the migration, `web-engineer` for the view; last in M8)*
...
- [x] **M8.5** (landed 2026-09-18: `/1v1` reads counted Summoner's Rift customs. Each lane lists the five longest same-role opposite-side series past three meetings; Lane bully is 70% over five; Dead heat is a score within one game over five. `?a=` and `?b=` pick any two people for enemies, allies, same-lane, KDA, form and a verdict. Default window `All time`. ARAM and KIWI are dropped. **Does not change `/stats`, Nemesis, the awards, or the rating model.**) Lane 1v1 and head-to-head.

    > **Brief (product, 2026-09-18)**
    >
    > M8.1 put Nemesis on `/fun` and listed a head-to-head page out of scope until the group asked twice.
    > They asked. This is that page. It is **not** M5.34 — that id on this main is "the week starts on Sunday".
    >
    > ### What a player sees
    >
    > A new tab **`1v1`** at `/1v1`, after `Fun` and before `Daily`. Same shell, same window picker. **Default
    > window is `All time`**. Week windows are Sunday 06:00 (M5.34). **Summoner's Rift only** — `countedGames`
    > then `matchesQueue(..., 'sr')`. No ARAM chip: Howling Abyss has no lanes. `/stats` still counts ARAM;
    > this page cannot.
    >
    > A lane meeting is **exactly one player on each side with that role**. Two tops on one side, or a missing
    > role, is not a 1v1 and does not count. Never guess a role. Each lane prints the **five** series with the
    > most meetings, past **three**. Winner's name first, `8W 2L · 80%`, both names links. A **percentage waits
    > for five meetings**, the same bar `/stats` uses.
    >
    > Two spice lists above the lanes: **Lane bully** (highest win rate, five meetings, at least 70%) and
    > **Dead heat** (score within one game, five meetings, longest first).
    >
    > **Pick two** is a GET form, `?a=` and `?b=` puuids, so the URL pastes into the chat. Two `<select>`s and a
    > Compare button; `onChange` submits when JavaScript is on. The series prints a verdict, a `12–7` score, a
    > brand/line bar, last meeting, streak, last-five form as `W W L W L`, mean KDA into each other, the
    > champion each locked, same-role rows, and the record when they queued together. Colour is never a side:
    > the leader's number is `text` at 600, the trailer is `dim`. Brand is the lead share of the bar.
    >
    > Every English card title carries an Egyptian 3ameya roast on the right in brand, the same dress `/fun`
    > uses. Copy lives in `lib/versus/copy.ts`.
    >
    > | English | Roast |
    > |---|---|
    > | Lane wars | حرب اللين |
    > | Lane bully | بيأكل اللين |
    > | Dead heat | محدش بيكسب |
    > | Pick two | هات اتنين |
    > | When they collide | مين بياكل التاني |
    > | When they queue together | لما بيلعبوا مع بعض |
    > | Same lane | نفس اللين |
    >
    > Page intro: `Who wins the lane, and who has whose number. Summoner's Rift customs only.`
    >
    > Empty / pick states: `Pick two people to open the series.` · `Pick the other player.` · `Pick two
    > different people.` · `Nobody has faced the same role often enough yet.` · `They have not played on
    > opposite sides in this window.` · `They have not queued on the same side in this window.`
    >
    > Verdicts: `They have not shared a custom in this window.` · `They only ever play together.` · `Dead even.`
    > · `{Winner} has {loser}'s number.` · `{Winner} leads the series.` · `They wreck the map together · {n}%.`
    > · `Cursed when stacked · {n}%.`
    >
    > ### Numbers, pinned
    >
    > - `MIN_LANE_GAMES = 3` before a pair appears on a lane board.
    > - `LANES_SHOWN = 5` rows per lane and per spice list.
    > - `TYRANT_RATE = 70` with `MIN_RECORD_GAMES` (5) meetings.
    > - A percentage uses `/stats`'s `winRate` helper (null under five).
    > - No new table. `game_players.role` and `winning_side` already answer it.
    >
    > ### Acceptance
    >
    > 1. Pure functions in `lib/versus/`, tested: a doubled top on one side is not a meeting; a missing role is
    >    not a meeting; ARAM and KIWI never appear in a lane series; a missing `gameMode` counts as Rift.
    > 2. `?a=` / `?b=` are puuids in a GET form; a repeated parameter 404s; picking one person prints the one
    >    empty line; picking the same person twice prints the same-person line.
    > 3. Default window is `All time` with no parameter; an unknown `?window=` is a 404.
    > 4. Nav lists `1v1` after `Fun` and before `Daily`; the tab underlines only on `/1v1`.
    > 5. `/stats`, Nemesis, Best duo, the awards, the board and the rating model are untouched.
    > 6. `pnpm -r typecheck`, `pnpm -r test` and `pnpm --filter web build` pass; the status table is updated;
    >    copy rows are in `05-design.md`.
    >
    > ### Out of scope
    >
    > Champion-level rivalries. A nemesis line on `/p/[puuid]`. An ARAM chip. A `matchups` table. Any change to
    > `duoRecords`, `compareDuos`, the awards, or `packages/core`.

Acceptance: `/fun` tells a person who beats them and who they win with, and names the nights the bot said they
would lose and they did not; last week's winners are labelled where the group actually looks; every other
morning the thing to guess is which of the ten owns a stat line, scored by the same function that decides who
carried a game; and `/1v1` answers who wins the lane and who has whose number. Nothing in this milestone moved
anybody's rating by one digit.

## M10 Fearless draft (half a day, needs M3 Discord and M2 ingest)

The user, 2026-09-18: after each game, list the ten champions that were played on the tonight page as bans
for the next custom, keep appending until an admin resets the list, and share it in Discord with the squad.

Fearless is a house rule, not a client action. **The companion never auto-bans** — Riot's line is that we do
not touch champion select. Humans ban from the list.

- [x] **M10.1** Fearless pool: unique Rift champions since the last admin reset, on `/`, in Discord after
  each rated game, cleared from `/admin`. *(owners: `platform-engineer` + `web-engineer`)*

    > **Brief (product, 2026-09-18)**
    >
    > ### What a player sees
    >
    > A card on the tonight page headed **Fearless**, sentence `Ban these next game.`, then the champion
    > names in first-appearance order. Hidden while the pool is empty. After a rated Summoner's Rift custom
    > Discord gets a second message (accent bar, same as teams) with the same list. An admin taps
    > **Reset fearless** on `/admin` and the pool is empty; Discord is told.
    >
    > ### Shape
    >
    > - Singleton `fearless_state` (`id = 1`, `reset_at`, `reset_by`). Migration `0017`. `reset_at` defaults
    >   to `now()` so deploying does not dump history into the first list.
    > - The pool is **derived**: unique `game_players.champion_id` from counted Rift games with
    >   `started_at > reset_at`. No champ table to double-append.
    > - ARAM is out (`matchesQueue(..., 'sr')`). Remakes are out (`gateGame`). A missing mode is Rift.
    > - Public read, service-role write, published to Realtime so a reset replaces the card live.
    >
    > ### Acceptance
    >
    > 1. Two Rift customs after a reset list twenty unique champs in first-appearance order; a repeat champ
    >    from game two does not appear twice (test).
    > 2. An ARAM and a remake add nothing (test).
    > 3. The tonight card is absent when the pool is empty and prints `Ban these next game.` plus the names
    >    when it is not (test).
    > 4. Discord's fearless embed is a second message, never a field on the result embed (test).
    > 5. `POST /api/admin/fearless/reset` is session + `is_admin`; anon cannot write the cursor (test).
    > 6. The companion still has no champion-select write. `pnpm -r typecheck` and the web tests pass;
    >    status table and decision row updated.
    >
    > ### Out of scope
    >
    > Auto-banning in the client. Per-night auto-clear at 06:00 (the user asked for admin reset). ARAM.
    > Storing a second copy of champion names.

- [x] **M10.2** Fearless list is searchable: grouped by the lane of first lock (`top` jungle `mid` `adc`
  `support`), A–Z inside each lane, find box on `/` that highlights an exact name. Discord uses the same
  lane fields. No champion-select read. *(owner: `web-engineer`)*

    > **Brief (product, 2026-09-20)**
    >
    > After four or five games the lock-order dump is unreadable. The pool is unchanged: unique ids,
    > first appearance still decides *whether* a champ is on the list and *which lane heading* it sits
    > under (`game_players.role` on that first seat, never guessed from the champion). Display sorts
    > lane then name. The tonight card has `Find a champion`; an exact match prints
    > `<Name> is on the ban list.` and paints that chip brand. Discord posts one field per non-empty
    > lane instead of one `Champions` column.
    >
    > ### Acceptance
    >
    > 1. Two champs first locked mid then adc print under `mid` then `adc`, A–Z inside the lane (test).
    > 2. A later game on a different role does not move the chip (test).
    > 3. Typing a name hides the rest and an exact name says it is on the ban list (test).
    > 4. The companion still has no champion-select path.
    >
    > ### Out of scope
    >
    > Reading `/lol-champ-select/*` to highlight live locks. Inferring a lane from the champion id.

- [x] **M10.3** Each fearless lane on `/` ends with the champions still open in that lane. *(owner: `web-engineer`)*

    > **Brief (the user, 2026-09-22)**
    >
    > After the bans in a lane, list who that lane can still lock, so a pick does not mean scrolling
    > the bans and then remembering the rest of the roster. The ban chip stays on the lane of first
    > lock. The open tail is a primary lane on the roster, one lane per champion, and a locked id
    > leaves every lane. The find box says `<Name> is still available.` when the exact name is not
    > banned. Discord stays the ban list — the open tail is the whole roster and would bury the bans.
    >
    > ### Acceptance
    >
    > 1. A mid ban and an adc ban leave Annie under mid's `still open` and Garen under top's, and
    >    neither locked name appears in an open chip (test).
    > 2. Typing `garen` says `Garen is still available.` and hides the bans (test).
    > 3. Every id in the champion name table has exactly one lane, and the lane table names no extra
    >    id (test).
    > 4. Discord's fearless embed is unchanged: bans only.
    > 5. The companion still has no champion-select path.
    >
    > ### Out of scope
    >
    > Filing a flex champion under every lane they can play. Reading champ select. Putting the open
    > tail in the Discord post.

Acceptance: after a Rift custom the tonight page and Discord both name the champs to ban next game,
...
- [ ] **M9.1** (**deferred to next release, the user, 2026-09-15** — do not dispatch even once M7.11 lands) Measure it: how often does somebody come back after a long break, and does their next game move them harder than anyone else's? A query and a written report. **No behaviour changes and no rating moves.** *(owner: `platform-engineer`; after **M7.11**, because the numbers have to be read out of the model the group is actually on)*
...
M8.1 (deferred the page) ---------------- M8.5 (`/1v1` lane wars and head-to-head)
