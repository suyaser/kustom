...
- [ ] **M7.19** (code half landed 2026-09-16, reviewed: `provisionalSeed()` in `packages/core`, the new
  `config.rating.provisionalSigma`, `seedFor`'s fallback, the two inline copies in `lib/ingest/rating.ts` folded
  into it, and every player-facing rating for a player with no `ratings` row. **Two steps are open**: the hosted
  re-seed plus one `rebuild-ratings` run, and product's re-wording of the two sentences that still say a rating
  comes from a rank — `/p/[puuid]`'s seed line and the shell's `How this works`.) **Nobody's rating starts
  ...
  ✓ — **this is the second and last rebuild the milestone owes**)*
    ...
    >    `05-design.md`'s copy table before merge.
